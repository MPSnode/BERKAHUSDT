...
export default function ExchangeCalculator({ buyRate: propBuy, sellRate: propSell }) {
  const { content, social, networks: cfgNetworks, paymentMethods: cfgPayments, freeFeeThresholdUsdt } = useSite();
  ...
  const banks = (cfgPayments && cfgPayments.length > 0
    ? cfgPayments.map((m) => ({ id: m.code, name: m.name || m.code, color: 'text-emerald-400' }))
    : [
  ...
  const networks = (cfgNetworks && cfgNetworks.length > 0
    ? cfgNetworks.map((n) => ({
        id: n.code,
        name: n.name || n.code,
        feeUsdt: Number(n.feeUsdt) || 0,
        fee: `${Number(n.feeUsdt) || 0} USDT`,
        minUsdt: 5,
        time: n.estimate || '1-3 Menit',
      }))
    : [{ id: 'TRC-20', name: 'Tron (TRC-20)', feeUsdt: 1, fee: '1 USDT', minUsdt: 5, time: '1-3 Menit' }]);

  React.useEffect(() => {
    if (banks.length > 0 && !banks.find((b) => b.id === selectedBank)) setSelectedBank(banks[0].id);
    if (networks.length > 0 && !networks.find((n) => n.id === selectedNetwork)) setSelectedNetwork(networks[0].id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cfgPayments, cfgNetworks]);

  const feeThreshold = Number(freeFeeThresholdUsdt) || 2000;

  const currentNetworkObj = networks.find(n => n.id === selectedNetwork) || networks[0];
  const isFreeFee = mode === 'buy' ? usdtAmount >= feeThreshold : usdtAmount >= feeThreshold;
  ...
  const waBase = social?.whatsapp || 'https://wa.me/6281234567890';
  const waLink = `${waBase}${waBase.includes('?') ? '&' : '?'}text=${getWaMessage()}`;
  const buildTelegramLink = (baseUrl) => {
    if (!baseUrl) return '';
    return `${baseUrl}${baseUrl.includes('?') ? '&' : '?'}text=${getWaMessage()}`;
  };
  const telegram1 = buildTelegramLink(social?.telegramAdmin1);
  const telegram2 = buildTelegramLink(social?.telegramAdmin2);
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          ...
          <h2 className="text-3xl sm:text-5xl font-extrabold text-white tracking-tight font-['Space_Grotesk']">
            {content?.calculatorTitle ? (
              <span>{content.calculatorTitle}</span>
            ) : (
              <span>Hitung Penukaran <span className="text-gradient-emerald">USDT Real-Time</span></span>
            )}
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between text-[11px] text-slate-400 font-mono pt-1">
                <span>
                  {isFreeFee ? (
                    <span className="text-emerald-400 font-bold">*Bebas Biaya Gas (Nominal ≥ {feeThreshold.toLocaleString('id-ID')} USDT)</span>
                  ) : (
                    <span>
              ...
              >
                {networks.map((net) => (
                  <option key={net.id} value={net.id}>
                    {net.name} — Fee: {isFreeFee ? `GRATIS (>= ${feeThreshold.toLocaleString('id-ID')} USDT)` : net.fee}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="flex justify-between items-center text-slate-300">
              ...
              {isFreeFee ? (
                <span className="font-extrabold text-emerald-400 font-mono flex items-center gap-1.5 bg-emerald-500/20 px-2.5 py-1 rounded-lg border border-emerald-500/30">
                  ...
                  Rp 0 (GRATIS FEE ≥ {feeThreshold.toLocaleString('id-ID')} USDT)
                </span>
              ) : (
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {telegram1 && (
              <a
                ...
                data-testid="cta-telegram-admin1"
                className="py-4 px-6 rounded-2xl bg-gradient-to-r from-sky-500 to-cyan-600 hover:from-sky-400 hover:to-cyan-500 text-white font-extrabold text-sm sm:text-base shadow-xl shadow-sky-950/50 hover:scale-[1.01] transition-all flex items-center justify-center gap-3"
              >
                <Send className="w-5 h-5" />
                <span>{social?.telegramAdmin1Label || 'PESAN KE TELEGRAM ADMIN 1'}</span>
              </a>
            )}
            {telegram2 && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                data-testid="cta-telegram-admin2"
                className="py-4 px-6 rounded-2xl bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-400 hover:to-emerald-500 text-white font-extrabold text-sm sm:text-base shadow-xl shadow-teal-950/50 hover:scale-[1.01] transition-all flex items-center justify-center gap-3"
              >
                <Send className="w-5 h-5" />
                <span>{social?.telegramAdmin2Label || 'PESAN KE TELEGRAM ADMIN 2'}</span>
              </a>
            )}
          </div>
